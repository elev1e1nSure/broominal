...
Или скачайте последнюю `.exe` из [релизов][releases].
...
Типичный путь: нашли → посмотрели → почистили → откатили.
...
# что-то пошло не так? восстановите последнюю очистку
broominal restore last
```

Для интерактивного режима запустите `broominal ui`.
...
- **отменяемо** — восстановить любую очистку по ID или вернуть последнюю
...
- **30+ категорий** — temp, кэши, логи, данные браузеров, инструменты разработки и не только
...
Просканируйте систему в поисках кандидатов на очистку по 30+ категориям.
...
Очистить выбранные элементы. По умолчанию чистятся элементы **safe** и **review**. Опасные элементы требуют явного подтверждения.

```powershell
# очистить безопасные и требующие проверки элементы (по умолчанию)
broominal clean
...
Восстановить предыдущую очистку. Каждая очистка получает уникальный ID для восстановления.
...
Запустить интерактивный TUI для guided-очистки.
...
Запустить проверки состояния для верификации работы broominal.
...
```powershell
# сгенерировать отчёт (запускает новое сканирование)
broominal report
```
...
> Включите общие githooks перед коммитами:
> ```powershell
